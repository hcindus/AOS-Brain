#!/usr/bin/env python3
"""Daily Queue Email Report - September 16, 2026 (live data @ 11:38 UTC)."""
import smtplib, ssl
...
════════════════════════════════════════════
  SYSTEM HEALTH
════════════════════════════════════════════
• Uptime:   120 days, 2h46m
• CPU Load: 1.27 / 1.07 / 1.02  🟢 healthy, settled
• Memory:   5.8 Gi used / 15 Gi total (38%)  🟢 healthy
• Available: 9.8 Gi
• Swap:     5.3 Gi / 29 Gi
• Disk:     137G / 193G (71%)  🟢 healthy

Core services: ✅ aos-brain-v4, aos-bhsi-v4, aos-mission-control, ollama,
and ✅ depotchaos — ALL ACTIVE (depotchaos now ACTIVE; resolved from
yesterday's "activating" state).
...
🟢 NOTE
  5. depotchaos is now ACTIVE (improvement from yesterday) — no action.
  ...
  7. System otherwise healthy: memory 38%, disk 71%, load settled
     ~1.0-1.3 — no OOM/disk/CPU risk.
...
services and mission-control not in ACTIVE state. depotchaos resolved.
