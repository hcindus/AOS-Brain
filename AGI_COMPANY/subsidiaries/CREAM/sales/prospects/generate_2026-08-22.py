...
def update_prospect_count(prospects: List[Dict]):
    ...
    prev_streak = data.get("streak_days", 112)
...
def update_marketing_materials(prospects: List[Dict]):
    ...
    for path in [f"{base}/BROCHURE.md", f"{base}/PITCH_DECK.md", f"{sales_base}/SALES_ENABLEMENT.md"]:
        try:
            ...
            # Update prospect count: 114,000 → 115,000
            old_values = ["114,000", "113,000+", "113,000", "112,000+", "112,000"]
            new_value = "115,000"
            for old in old_values:
                content = content.replace(old, new_value)
            content = content.replace("114,000+", "115,000")

            # Update streak
            old_streaks = ["112-day", "111-day", "110-day", "109-day"]
            for old in old_streaks:
                content = content.replace(old, "113-day")
            content = re.sub(r'(Streak\s*\|\s*)\d+(\s*days)', r'\g<1>113\g<2>', content)
            content = re.sub(r'(Streak.*?)\b(109|110|111)\b(.*?days)', r'\g<1>113\g<3>', content)
    ...
    try:
        with open(battle_path, 'r') as f:
            content = f.read()
        old_vals = ["114,000", "113,000+", "113,000", "112,000+"]
        for old in old_vals:
...
def main():
    ...
    print(f"Database: 114,000 → 115,000")
    print(f"🔥 Streak: Day 113 — Post-century momentum!")
    print("=" * 60)
    print()
    ...
    print(f"📈 Total prospects: 114,000 → 115,000 (+1,000)")
    print(f"🔥 Daily streak: 113 DAYS — Post-century momentum!")
