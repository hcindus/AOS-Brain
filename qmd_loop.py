...
class QMDLoop:
    ...
    def __init__(self, model: str = "tinyllama:latest", use_ollama: bool = False):
        self.model = model
    ...
    def _query_ollama(self, query: str, context: Dict, memories: List[Dict]) -> tuple:
        ...
        try:
            ...
            # Call Ollama with tinyllama-optimized settings
            response = requests.post(
                ...
                json={
                    ...
                    "options": {
                        ...
                        "num_predict": 15    # Short response for speed
                    }
                },
                timeout=15
            )
    ...
    def _build_prompt(self, query: str, context: Dict, memories: List[Dict]) -> str:
        """Build structured prompt for tinyllama (optimized for keyword output)"""
        limbic = context.get('limbic', {})
        
        prompt = f"""Choose ONE word: EXPLORE, EXPLOIT, REST, or CONTINUE

Novelty: {limbic.get('novelty', 0.5):.2f}
Reward: {limbic.get('reward', 0.3):.2f}
Phase: {context.get('phase', 'unknown')}
Observation: {context.get('observation', 'none')[:80]}
