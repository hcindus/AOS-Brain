import { c as NonInteractivePermissionPolicy, l as PermissionMode, o as AuthPolicy, p as PromptInput, s as McpServer$1, t as SessionAgentOptions, u as PermissionPolicy } from "./session-options-Bh1bIqQ2.js";
...
declare class FlowRunner {
  ...
  private executeFlowRun;
  private executeFlowStep;
  private writeNodeStartedSnapshot;
  private executeStartedFlowStep;
  private createSuccessfulFlowStep;
  private createFailedFlowStep;
  private maybeCompleteCheckpointStep;
  private resolveNextNode;
  private completeFlowRun;
  private recordFlowStepOutcome;
  private executeNode;
  private executeComputeNode;
  private executeActionNode;
  private executeCheckpointNode;
  private executeAcpNode;
  private prepareAcpPrompt;
  private writeAcpPromptHeartbeat;
  private executeIsolatedAcpPrompt;
  private initializeIsolatedSessionBundle;
  private executePersistentAcpPrompt;
  private appendAcpPromptPreparedTrace;
  private finishAcpPrompt;
  private writeAcpRawResponseArtifact;
  private appendAcpResponseParsedTrace;
  private parseAcpOutput;
...
//#endregion
//#region src/flows/decision.d.ts
type DecisionAcpOptions = Omit<AcpNodeDefinition, "nodeType" | "prompt" | "parse">;
type DecisionDefinition<TChoice extends string> = DecisionAcpOptions & {
  question: string | ((context: FlowNodeContext) => string | Promise<string>);
  choices: readonly TChoice[];
  field?: string;
};
declare function decision<TChoice extends string>(definition: DecisionDefinition<TChoice>): AcpNodeDefinition;
declare function decisionEdge<TChoice extends string>(args: {
  from: string;
  choices: readonly TChoice[];
  field?: string;
...
//#endregion
export { type AcpNodeDefinition, type ActionNodeDefinition, type CheckpointNodeDefinition, type ComputeNodeDefinition, type DecisionDefinition, type FlowDefinition, type FlowEdge, type FlowNodeCommon, type FlowNodeContext, type FlowNodeDefinition, type FlowPermissionRequirements, type FlowRunDefinition, type FlowRunResult, type FlowRunState, FlowRunner, type FlowRunnerOptions, type FlowSessionBinding, type FlowStepRecord, type FunctionActionNodeDefinition, type JsonObjectParseMode, type ResolvedFlowAgent, type ShellActionExecution, type ShellActionNodeDefinition, type ShellActionResult, acp, action, checkpoint, compute, decision, decisionEdge, defineFlow, extractJsonObject, flowRunsBaseDir, parseJsonObject, parseStrictJsonObject, shell };
