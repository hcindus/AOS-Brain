#!/usr/bin/env python3
"""
AOS COMPLETE BRAIN v4.2
Legacy + Ternary + Socket Interface + THYROID v1.1 + Model Router
...
- THYROID v1.1 (Reflexive mode switching + memory-aware)
- Model Router (tinyllama for decisions, Mort_II for voice)

NEW in v4.2: 
- Thyroid automatically switches between LOCAL and OLLAMA modes
- Model Router selects optimal LLM per task (tinyllama/Mort_II)
...
from thyroid_v11 import AOSThyroidV11, BrainMode
...
class BrainSocketServer:
    ...
    def _execute_command(self, cmd: str, params: dict) -> dict:
        ...
        if cmd == 'status':
            ...
            if self.brain.thyroid:
                approved = self.brain.thyroid.request_promotion(importance=importance)
                return {'promoted': approved, 'mode': self.brain.thyroid.state.current_mode.name}
            return {'error': 'Thyroid not available'}
        else:
            return {'error': f'Unknown command: {cmd}'}


class CompleteBrainV42:
    """
    Fully integrated brain with ALL features + Thyroid v1.1 + Model Router
    """
    
    def __init__(self, test_mode=False):
        print("=" * 70)
        print("  🧠 COMPLETE BRAIN v4.2")
        print("  Legacy + Ternary + Socket + THYROID + Model Router")
        print("=" * 70)
        ...
        # NEW: Thyroid v1.1 (with Model Router)
        print("[NEW v4.2] Thyroid v1.1 (memory-aware)...")
        self.thyroid = AOSThyroidV11(
            qmd_loop=self.qmd,
            check_interval=30.0,
        ...
        announcement = self.router.speak(
            "Complete Brain v4.2 initialized with Thyroid and Model Router",
            {"situation": "startup"}
        )
    ...
    def system_cycle(self):
        ...
        # 9. QMD decision (via Router if in OLLAMA mode)
        qmd_context = {
        ...
        # Use Router for decisions when in OLLAMA mode
        if self.thyroid.state.current_mode == BrainMode.OLLAMA:
            try:
        ...
        if self.tick_count % 50 == 0:
            ...
            thyroid_mode = self.thyroid.state.current_mode.name
            router_model = self.router.MODELS['decision'] if thyroid_mode == "OLLAMA" else "local"
            ...
            print(f"\n[Cycle {self.tick_count:5d}] "
                  ...
                  f"🫁 {thyroid_mode:6s} ({router_model.split(':')[0]}) | "
    ...
    def run(self):
        """Run complete system"""
        print("\n[SYSTEM] Complete Brain v4.2 running...")
        
        # Start thyroid monitor
        self.thyroid.start()
...
if __name__ == "__main__":
    print("\n" + "=" * 70)
    print("  🧠 COMPLETE BRAIN v4.2")
    print("  Thyroid v1.1 + Model Router + tinyllama/Mort_II")
    print("=" * 70)
    
    brain = CompleteBrainV42()
    ...
    try:
        ...
        print("=" * 70)
        print("  Complete Brain v4.2 Finished")
        print("=" * 70)
