...
BRAIN_PID=$(pgrep -f "complete_brain_v45" | head -1)
if [ -n "$BRAIN_PID" ]; then
    BRAIN_UPTIME=$(ps -p $BRAIN_PID -o etime= 2>/dev/null | xargs)
    echo "[$(date -u '+%Y-%m-%d %H:%M:%S UTC')] ✅ Complete Brain v4.5: RUNNING (PID $BRAIN_PID, uptime: $BRAIN_UPTIME)" | tee -a "$LOG_FILE"
else
    echo "[$(date -u '+%Y-%m-%d %H:%M:%S UTC')] ❌ Complete Brain v4.5: NOT RUNNING" | tee -a "$LOG_FILE"
