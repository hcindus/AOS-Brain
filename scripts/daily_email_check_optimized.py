#!/usr/bin/env python3
"""
Daily Email Checker for Miles (Optimized)
Checks miles@myl0nr0s.cloud for new emails specifically from Captain
"""
...
def check_emails():
    """Check inbox for Captain's emails only"""
    ...
    try:
        ...
        # Search for emails from Captain only (more efficient)
        status, messages = mail.search(None, f'FROM "{CAPTAIN_EMAIL}" UNSEEN')
        
        if status != "OK":
            log_message("No new messages from Captain")
            mail.logout()
            return
        ...
        if not message_ids:
            log_message("📭 No new actionable emails from Captain")
            mail.logout()
            return
        
        log_message(f"🎯 Found {len(message_ids)} new email(s) from Captain")
        ...
        for msg_id in message_ids:
            ...
            actionable_count += 1
            log_message(f"📧 ACTIONABLE: '{subject}'")
