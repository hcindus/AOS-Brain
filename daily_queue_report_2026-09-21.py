#!/usr/bin/env python3
"""Daily Queue Email Report - September 21, 2026 (live data @ 06:00 UTC)."""
...
════════════════════════════════════════════
  SYSTEM HEALTH
════════════════════════════════════════════
• Uptime:   124 days, 21h09m
• CPU Load: 2.09 / 1.65 / 1.53  🟢 healthy, settled
• Memory:   7.5 Gi used / 15 Gi total (50%)  🟢 improved (was 57% yesterday)
• Available: 8.1 Gi
• Swap:     1.4 Gi / 29 Gi
• Disk:     138G / 193G (72%)  🟢 healthy

Core services: ✅ aos-brain-v4, aos-bhsi-v4, aos-mission-control, ollama
all ACTIVE. ⚠️ depotchaos still "activating" (third consecutive day).
...
🟡 IMPORTANT
  2. depotchaos stuck in "activating" for a third day — worth a
  ...
  4. PENDING_TASKS lastUpdated still has not refreshed today (still
     09-19 11:16 UTC). The scraper/refresher did not run again — check
     that scheduled job.

🟢 NOTE
  5. Memory improved again — 57% → 50% (8.6 Gi → 7.5 Gi used). Trend
     back to healthy; no action needed.
  6. DB size and ca_abc_licenses count both unchanged from yesterday —
     no new ingest, no growth concern.
  7. System otherwise healthy: disk 72%, load ~1.5-2.1 — no OOM/disk/CPU
     risk.
...
"activating" for a third day, the four failed services, and
PENDING_TASKS not refreshing. Standing by.
